...
from ..rename.rakuten import rename_pdf
...
def read_params_with_gui(
    ...
    w = sg.Window(
        ...
        [
            [
                sg.Text('pdf 保存先ディレクトリ', size=text_size), sg.Input(p.get('store_path', '')),
                sg.FolderBrowse('参照')
            ],
            [sg.Text('ユーザーIDまたはメールアドレス', size=text_size), sg.Input(p.get('email', ''))],
            [sg.Text('パスワード', size=text_size), sg.Input(p.get('password', ''), password_char='*')],
            [sg.Text('領収書 宛名', size=text_size), sg.Input(p.get('name', ''))],
            [
    ...
    if event == '変更開始':
        ...
        if licensed:
            for i in values[0].split(';'):
                rename_pdf(i, p['rename_path'])
                sleep(1)
        else:
            rename_pdf(values[0], p['rename_path'])
            sleep(3)
        rename_window_gui(licensed, title, icon, prefix, load_params, save_params)
    else:
        logger.info('終了します。')
